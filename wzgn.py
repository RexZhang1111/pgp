...
def process_all_videos():
    ...
    if not cut_seconds_str or not output_duration_str or not clip_count_str:
        messagebox.showerror("错误", "请输入剪切秒数、输出持续时间和剪辑数量！")
        return
    ...
    try:
        ...
        messagebox.showerror("错误", "剪切秒数、输出持续时间和剪辑数量必须为数字！")
        return

    if cut_seconds <= 0 or output_duration <= 0 or clip_count <= 0:
        messagebox.showerror("错误", "剪切秒数、输出持续时间和剪辑数量必须大于0！")
        return
    ...
    while i <= clip_count:
        try:
            ...
            for j in range(len(segment_start_times)):
                ...
                command_segment = f"-i \"{segment_temp_file}\""
                command_segments.append(command_segment)
                clips.append(mpe.VideoFileClip(segment_temp_file))

            # 对生成的视频进行随机水平翻转、增加画面对比度和丢帧操作
            from moviepy.editor import fx

            flip_probability = 0.5
            contrast_change_range = (-0.5, 0.5)
            frame_drop_probability = 0.1

            for clip in clips:
                # 随机水平翻转
                if random.random() < flip_probability:
                    clip = clip.fx(fx.vfx.flip_horizontal)

                # 随机增加画面对比度
                contrast_change = random.uniform(*contrast_change_range)
                clip = clip.fx(fx.color.ColorCorrection(contrast=contrast_change))

                # 随机丢帧
                drop_frames = int(len(clip) * frame_drop_probability)
                if drop_frames > 0:
                    indices_to_keep = set(range(len(clip)))
                    while len(indices_to_keep) > (len(clip) - drop_frames):
                        index_to_remove = random.randint(0, len(clip) - 1)
                        indices_to_keep.remove(index_to_remove)
                    new_clip_frames = [clip.get_frame(i) for i in sorted(indices_to_keep)]
                    clip = mpe.VideoClip(new_clip_frames, clip.duration)
